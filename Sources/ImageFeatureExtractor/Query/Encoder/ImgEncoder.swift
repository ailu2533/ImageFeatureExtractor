...
enum ImageEncodingError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .encodingFailed(error):
            "图片编码失败：\(error.localizedDescription)"
        case .resizeError:
            "图片调整大小失败"
        case .bufferConversionError:
            "图片缓冲区转换失败"
    ...
    var recoverySuggestion: String? {
        ...
        case .fileNotFound:
            "请检查文件路径是否正确，确保文件存在"
        case .invalidImageData:
            "请确保图片格式正确且未损坏，支持的格式包括：JPEG、PNG"
        case .transparencyRemovalFailed:
            "请尝试使用不包含透明度的图片，或确保图片格式正确"
        case .encodingFailed:
            "请检查图片大小和格式是否符合要求，建议使用小于 4MB 的图片"
        case .resizeError:
            "请确保图片尺寸合理，建议使用 224x224 或更小的图片"
        case .bufferConversionError:
            "请确保图片格式正确，建议使用 RGB 格式的图片"
        case .featureProviderError:
            "请确保图片预处理正确，检查图片是否为空"
        case .predictionError:
            "请确保模型加载正确，检查系统内存是否充足"
        }
    }
}
